...
{
    if (!camId.compare("topCam"))
        return Vector(0, -5, 0);
    else if (!camId.compare("faceCam"))
        return Vector(-5, 0, 0);
    else if (!camId.compare("backCam"))
        return Vector(5, 0, 0);
    else if (!camId.compare("gaucheCam"))
        return Vector(0, 0, 5);
    else if (!camId.compare("droitCam"))
        return Vector(0, 0, -5);
    else
        return Vector(0, 0, 0);
}
...
int main(int argc, char *argv[])
{
    ...
    while (!(keyboard.ctrl.pressed && keyboard.q.down))
    {
        ...
        if (HomeDesign::interactSpace != 4)
        { // Hors mode insertion meuble
